...
const UserQueryAnalysisSchema = z.object({
    userInputLanguageCode: z.string().describe('ISO 639-1 language code for the user query'),
    userInputLanguageName: z.string().describe('ISO 639-1 language name for the user query'),
    questionTranslatedToEnglish: z.string().describe('The user\'s question, translated to English'),
...
export async function userInputAnalysis(userInput: string): Promise<UserQueryAnalysis> {
    ...
    if (envVar('USE_AZURE_OPENAI_API', false) === 'true') {
        ...
        console.log(`${stageName} model name: ${envVar('OPENAI_API_MODEL_NAME')}`);
