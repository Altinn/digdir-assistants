...
export async function lookupSearchPhrasesSimilar(relaxedQueries: QueryRelaxation): Promise<RankedUrl[]> {
    ...
    searchPhraseHits.sort((a, b) => b['hybrid_search_info']['rank_fusion_score'] - a['hybrid_search_info']['rank_fusion_score']);
    ...
    const uniqueUrls = urlList.filter((value, index, self) =>
