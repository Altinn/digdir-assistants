...
import { lookupSearchPhrasesSimilar, retrieveAllByUrl } from './retrieval-typesense'
...
export async function ragPipeline(user_input: string,
    user_query_language_name: string,
    stream_callback_msg1: any = null,
    stream_callback_msg2: any = null): Promise<RagPipelineResult> {
    ...
    const search_phrase_hits = await lookupSearchPhrasesSimilar(extract_search_queries);
    ...
    if (!isValidUrl(rerankUrl)) {
        throw new Error(`Environment variable 'COLBERT_API_URL' is invalid: '${rerankUrl}'`);
    }
    ...
    while (docIndex < searchHitsReranked.length) {
        ...
        if ((docsLength + docTrimmed.length) > envVar("MAX_CONTEXT_LENGTH")) {
            docTrimmed = docTrimmed.substring(0, envVar("MAX_CONTEXT_LENGTH") - docsLength - 20);
        }
        ...
        console.log(`loaded markdown doc, length= ${docTrimmed.length}, url= ${uniqueUrl}`);
        ...
        if (docsLength >= envVar("MAX_CONTEXT_LENGTH") || loadedDocs.length >= envVar("MAX_CONTEXT_DOC_COUNT")) {
    ...
    console.log(`Starting RAG structured output chain, llm: ${envVar("OPENAI_API_MODEL_NAME")}`);
    start = performance.now();
    ...
    const fullPrompt = qaTemplate().replace('{context}', contextYaml).replace('{question}', user_input);
    ...
    if (translation_enabled && rag_success && user_query_language_name !== 'English') {
        translated_answer = await translate(english_answer, user_query_language_name, stream_callback_msg2); 
    }
