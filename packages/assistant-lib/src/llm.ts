...
export function extract_json_from_response(response: string, json_doc_keyword: string): JsonExtraction {


    if (response.toLowerCase().includes(json_doc_keyword.toLowerCase())) {
        const keyword_start = response.toLowerCase().indexOf(json_doc_keyword.toLowerCase());
        const json_doc_start = keyword_start + json_doc_keyword.length;
        let json_str = response.substring(json_doc_start).trim();

        if (json_str.startsWith('```') && json_str.endsWith('```') && json_str.includes('\n')) {
            json_str = json_str.substring(json_str.indexOf('\n'), json_str.length - 3).trim();
        }
    ...
    return JsonExtractionSchema.parse({ json_dict: {}, response_json_removed: response });
}
...
export async function chat_stream(messages: Array<ChatCompletionMessageParam>,
    callback: (arg0: string) => void,
    callback_interval_seconds = 2.0): Promise<string> {
