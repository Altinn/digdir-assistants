...
export const getToken = async (): Promise<Session | null> => {
  ...
  try {
    ...
    console.error(
      "getToken(): Error retrieving token from Supabase session.",
      error,
    );
    return null;
  }
...
export const isLoggedIn = async () => {
  ...
  try {
    ...
    if (!session.expires_at) {
      console.error(
        "isLoggedIn(): Session object does not contain 'expires_at' field.",
      );
      return false;
    }
    if (currentTime > expiration) {
      console.log(
        `isLoggedIn(): Current time: ${currentTime}, Expires at: ${expiration} Token has expired, logging out user.`,
      );
      logout();
      return false;
    }
...
export const logout = () => {
  ...
  supabase.auth
    ...
    .then(() => {
      console.log(
        "Supabase auth session ended successfully, redirecting to site root...",
      );
      window.location.href = "/";
    })
    .catch((error) => {
      console.error(
        "Error logging out of Supabase auth session:",
        error.message,
      );
    });
};
