...
const supabase: SupabaseClient = createClient(envVar('SLACK_BOT_SUPABASE_URL'), envVar('SLACK_BOT_SUPABASE_API_KEY'));
...
export async function updateReactions(slackContext: SlackContext, reactions: any) {
    ...
    const resultSet = await supabase.from('bot_log').select("*").eq("slackContext->>ts", ts).eq("slackContext->>channel", channel);
