...
async function main() {
  ...
  while (true) {
    ...
    const query = `
    SELECT dp.document_id, an.actor_id as actor_id, an2.name as abbreviation, an.name as actor_name 
    FROM document_publisher dp 
    JOIN actor_names an ON dp.actor_id = an.actor_id 
    JOIN actor_names an2 ON dp.actor_id = an2.actor_id 
  ...
  `;
    ...
    const updatedDocs = rows.map(row => ({
      ...
      publisher_short: row.abbreviation,
      publisher_long: row.actor_name
    }));
