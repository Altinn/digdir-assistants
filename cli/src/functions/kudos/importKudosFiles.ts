...
async function main() {
  ...
  program
    .name('kudosImportFiles')
    .description(
      'Import KUDOS document files from a local MariaDB instance to an existing Typesense docs collections',
    )
    .version('0.1.0');
  ...
  program
    ...
    .requiredOption('-c, --collection <string>', 'typesense collection name for file metadata (not alias name)', '')
  ...
  if (!opts.dryrun) {
    ...
    console.log(`Will update source file metadata in collection: '${docsCollectionName}'`);
  }
  ...
  while (true) {
    ...
    const query = `
    SELECT \`id\`, \`document_id\`, \`size\`,
    CONCAT('https://kudos.dfo.no/documents/', document_id, '/files/', id, '.pdf')
       AS \`url\`
    FROM
      \`files\`
  ...
  `;
    ...
    const updatedDocs = rows.map(row => ({
      id: row.id.toString(),
      file_id: row.id.toString(),
      doc_num: row.document_id.toString(),
      size: row.size,
      kudos_url: row.url,
    }));
    ...
    if (!opts.dryrun) {
      try {
        await typesenseClient
          ...
          .documents()
          .import(updatedDocs, { action: 'upsert' });

        console.log(`Upserted file metadata, page ${page}`);
      } catch (error) {
