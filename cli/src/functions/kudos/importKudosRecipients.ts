...
async function main() {
  ...
  program
    .name('kudosImportRecipients')
    .description(
      'Import KUDOS document recipients from a local MariaDB instance to an existing Typesense docs collections',
    )
    .version('0.1.0');
  ...
  if (!opts.dryrun) {
    ...
    console.log(`Will update recipient names in collection: '${docsCollectionName}'`);
  }
  ...
  `;
    ...
    const updatedDocs = rows.map(row => ({
      ...
      recipient_short: row.abbreviation ? row.abbreviation : row.name_long,
      recipient_long: row.official ? row.official : row.name_long,
    }));
    ...
    if (!opts.dryrun) {
      try {
        ...
        console.log(`Updated recipients on page ${page}`);
      } catch (error) {
        console.error('Error updating recipients for docs:', error);
      }
    }
    page++;
  }
