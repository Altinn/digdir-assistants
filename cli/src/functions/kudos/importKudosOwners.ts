...
async function main() {
  ...
  program
    .name('kudosImportOwners')
    .description(
      'Import KUDOS document owners from a local MariaDB instance to an existing Typesense docs collections',
    )
    .version('0.1.0');
  ...
  if (!opts.dryrun) {
    ...
    console.log(`Will update owner names in collection: '${docsCollectionName}'`);
  }
  ...
  `;
    ...
    if (!opts.dryrun) {
      try {
        ...
        console.log(`Updated owners on page ${page}`);
      } catch (error) {
        console.error('Error updating owners for docs:', error);
      }
    }
    page++;
  }
